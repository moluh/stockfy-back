        ...
        const csvStream = fastcsv
            ...
            .on('end', async function () {
                ...
                let header: any = {
                    ...
                    stock_actual: csvData[0].findIndex(
                        (h) => h === 'stock_actual'
                    ),
                    ...
                    descripcion: csvData[0].findIndex(
                        (h) => h === 'descripcion'
                    ),
                    disponible: csvData[0].findIndex((h) => h === 'disponible'),
                    rebaja: csvData[0].findIndex((h) => h === 'rebaja'),
                    precio_costo: csvData[0].findIndex(
                        (h) => h === 'precio_costo'
                    ),
                    precio_venta: csvData[0].findIndex(
                        (h) => h === 'precio_venta'
                    ),
                    ...
                    profundidad: csvData[0].findIndex(
                        (h) => h === 'profundidad'
                    ),
                        ...
                        if (csvData[i][header.id] !== '') {
                            await getConnection()
                                ...
                                .values({
                                    ...
                                    rebaja:
                                        parseFloat(csvData[i][header.rebaja]) ||
                                        0,
                                    precio_costo:
                                        parseFloat(
                                            csvData[i][header.precio_costo]
                                        ) || 0,
                                    precio_venta:
                                        parseFloat(
                                            csvData[i][header.precio_venta]
                                        ) || 0,
                                    sku: csvData[i][header.sku],
                                    codigo_fabricante:
                                        csvData[i][header.codigo_fabricante],
                                    unidad: csvData[i][header.unidad],
                                    archivado: csvData[i][header.archivado],
                                    alto:
                                        parseInt(csvData[i][header.alto]) || 0,
                                    ancho:
                                        parseInt(csvData[i][header.ancho]) || 0,
                                    profundidad:
                                        parseInt(
                                            csvData[i][header.profundidad]
                                        ) || 0,
                                    peso:
                                        parseInt(csvData[i][header.peso]) || 0,
                                })
                                .orUpdate({
                                    conflict_target: ['id'],
                                    overwrite: [
