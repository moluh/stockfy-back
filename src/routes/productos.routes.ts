...
export class ProductosRouter {
    ...
    public routes(app): void {
        ...
        app.route('/api/v1/producto/state/:id').post(
            mw.isAllowed([SUPERADMIN]),
            this.controlador.changeState
        )

        app.route('/api/v1/producto/barcodes/:id/sku/:sku/ean/:ean').post(
            mw.isAllowed([SUPERADMIN]),
            this.controlador.createBarCodes
        )

        app.route('/api/v1/productos/paginado/state').get(
            mw.isAllowed([EMPLEADO, SUPERADMIN]),
            this.controlador.getPaginatedByState
        )

        app.route('/api/v1/productos/paginado/list').get(
            mw.isAllowed([EMPLEADO, SUPERADMIN]),
            this.controlador.getPaginatedByIdOfAList
        )

        app.route('/api/v1/productos/paginado/filter').get(
            mw.isAllowed([EMPLEADO, SUPERADMIN]),
        ...
        app.route('/api/v1/producto/barcode/:barcode').get(
            mw.isAllowed([EMPLEADO, SUPERADMIN]),
            this.controlador.getByCodes
        )
    }
}
