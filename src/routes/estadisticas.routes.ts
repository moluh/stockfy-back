...
export class EstadisticasRouter {
    ...
    public routes(app): void {
        app.route('/api/v1/estadisticas').post(
            mw.isAllowed([SUPERADMIN]),
            this.controlador.getBetweenDates
        )
        app.route('/api/v1/estadisticas/grafico').post(
            mw.isAllowed([SUPERADMIN]),
            this.controlador.getBetweenDatesGraphic
        )

        app.route('/api/v1/dashboard').get(
            mw.isAllowed([USUARIO]),
